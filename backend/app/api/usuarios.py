...
def read_usuarios(
    ...
    if sede_id is not None:
        query = query.outerjoin(Grupo, Usuario.grupo_id == Grupo.id).filter(
            or_(
                Usuario.sede_id == sede_id,
                Grupo.sede_id == sede_id
            )
        )
...
def create_usuario(
    ...
    if usuario.grupo_id and not sede_id:
        grupo = db.query(Grupo).filter(Grupo.id == usuario.grupo_id).first()
        if grupo and grupo.sede_id:
...
def update_usuario(
    ...
    if 'grupo_id' in update_data and update_data['grupo_id'] is not None and ('sede_id' not in update_data or update_data['sede_id'] is None):
        grupo = db.query(Grupo).filter(Grupo.id == update_data['grupo_id']).first()
        if grupo and grupo.sede_id:
            update_data['sede_id'] = grupo.sede_id
