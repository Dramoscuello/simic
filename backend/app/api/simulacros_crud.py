...
def read_simulacros(
    ...
    batch_id: Optional[str] = None,  # Filtro por lote
    ...
    if rol_nombre == 'estudiante':
        ...
        if batch_id:
            query = query.filter(Simulacro.batch_id == batch_id)
...
async def delete_simulacro(
    ...
    print(f"\n{'='*60}")
    print(f"  ELIMINANDO SIMULACRO: {simulacro_id}")
    print(f"   Título: {db_simulacro.titulo}")
    print(f"   Usuario: {current_user.email}")
    print(f"{'='*60}")
    
    try:
        # 1. Eliminar revisiones de preguntas asociadas
        reviews_eliminadas = db.query(ReviewPregunta).filter(
            ReviewPregunta.simulacro_id == simulacro_id
        ).delete(synchronize_session=False)
        print(f"    Revisiones de preguntas eliminadas: {reviews_eliminadas}")

        # 2. Eliminar reportes grupales asociados
        reportes_eliminados = db.query(ReporteGrupal).filter(
            ReporteGrupal.simulacro_id == simulacro_id
        ).delete(synchronize_session=False)
        print(f"    Reportes grupales eliminados: {reportes_eliminados}")

        # 3. Eliminar respuestas de estudiantes asociadas
        ...
        # 4. Eliminar preguntas_usadas asociadas
