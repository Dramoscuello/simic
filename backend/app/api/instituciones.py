...
async def import_usuarios(
    ...
    if not _is_super_admin(current_user) and current_user.institucion_id != institucion_id:
    ...
    for index, row in df.iterrows():
        try:
            ...
            user_sede_id = None
            if grupo_id:
                grupo_obj = db.query(Grupo).filter(Grupo.id == grupo_id).first()
                if grupo_obj and grupo_obj.sede_id:
                    user_sede_id = grupo_obj.sede_id
