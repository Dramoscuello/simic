...
class GenerateRequest(BaseModel):
    ...
    batch_id_continue: Optional[str] = None  # Si se continúa un batch existente, reutiliza ese batch_id
...
def _sociales_target_distribution(
    ...
    """
    comp_ratios = {
        "Pensamiento social": 0.30,
        "Interpretación y análisis de perspectivas": 0.40,
...
def generate_simulacros_async(
    ...
    try:
        ...
        import traceback
        print(f"ERROR in generate_async: {type(e).__name__}: {e}")
        traceback.print_exc()
...
def run_generation_job(job_id: str, request_dict: dict, db_url: str):
    ...
    try:
        ...
        # Resolver o generar batch_id
        def _sanitize_batch(s: str) -> str:
            return re.sub(r'[^a-z0-9]+', '-', s.lower().strip())[:60]

        batch_id_continue = request_dict.get("batch_id_continue")
        if batch_id_continue:
            existing = db.query(Simulacro).filter(
                Simulacro.batch_id == batch_id_continue,
                Simulacro.institucion_id == institucion_id
            ).first()
            if not existing:
                raise ValueError(f"Batch {batch_id_continue} no encontrado en esta institución")
            batch_id = batch_id_continue
            print(f" Continuando batch existente: {batch_id}")
        else:
            batch_id = f"{_sanitize_batch(nombre_base)}_{datetime.now().strftime('%Y%m%d%H%M')}"
            print(f" Nuevo batch creado: {batch_id}")
        ...
        for area in areas:
            ...
            print(f"    Timeout OpenAI configurado: {openai_timeout}s (num_preguntas={num_preguntas})")
            ...
            try:
                ...
                while not validation_passed:
                    ...
                    if area == "CIENCIAS_NATURALES":
                        ...
                        try:
                            ...
                            print(f" Gate 8 Error Interno: {str(e)}")
                    ...
                    if area == "INGLES":
                        ...
                        try:
                            ...
                            if not cefr_result.passed:
                             ...
                             import traceback
                             traceback.print_exc()
                             print(f" Gate 9 Error Interno: {str(e)}")
                    ...
                    if not gate_errors:
                        ...
                        if len(valid_questions) >= num_preguntas:
                            ...
                            if area == "INGLES":
                                ...
                                print(f"       Blindaje equilibrado Sociales:")
                                print(f"         Competencias: {comp_summary}")
                                print(f"         Dificultad: {dif_summary}")
                        ...
                        if preguntas_a_reparar:
                            ...
                            if repair_result.success and repair_result.data:
                                ...
                                print(f"       Validando estructura de {len(raw_preguntas)} preguntas reparadas...")
                                ...
                                for p_rep in raw_preguntas:
                                    try:
                                        ...
                                        p_id = p_rep.get('id', '?')
                                        print(f"       Pregunta reparada {p_id} DESCARTADA por error estructural: {e.errors()[0]['msg']}")
                            ...
                            print(f"       No hay preguntas específicas para reparar")
                            break
                ...
                if not validation_passed:
                    ...
                    if valid_percent >= 0.8:
                         print(f"\n    Máximo de intentos alcanzado, pero salvando simulacro parcial ({valid_percent*100:.0f}% OK).")
                ...
                if len(final_questions) > num_preguntas:
                    if area == "INGLES":
                        print(f"\n    Recortando excedente ({len(final_questions)} -> {num_preguntas}) con equilibrio por partes (INGLÉS)...")
                        ...
                        print(f"\n    Recortando excedente ({len(final_questions)} -> {num_preguntas}) con equilibrio por competencia+dificultad (MATEMÁTICAS)...")
                        ...
                        print(f"\n    Recortando excedente ({len(final_questions)} -> {num_preguntas}) con equilibrio por componente+dificultad (CIENCIAS)...")
                        ...
                        print(f"\n    Recortando excedente ({len(final_questions)} -> {num_preguntas}) con equilibrio por competencia+dificultad (SOCIALES)...")
                        ...
                        print(f"       Distribución final Sociales:")
                        print(f"         Competencias: {comp_summary}")
                        print(f"         Dificultad: {dif_summary}")
                        contenido["preguntas"] = seleccionadas
                    elif area == "LECTURA_CRITICA":
                        print(f"\n    Recortando excedente ({len(final_questions)} -> {num_preguntas}) con equilibrio por afirmacion+dificultad (LECTURA)...")
                        ...
                        print(f"\n    Recortando excedente ({len(final_questions)} -> {num_preguntas}) respetando dificultad...")
                        ...
                        if faltantes > 0:
                            print(f"       Faltaron preguntas para cubrir cuotas exactas. Rellenando con {faltantes} de otros niveles.")
                ...
                if job_lock:
                    ...
                    if released:
                        ...
                        print(f"    Lock no liberado (sin ownership o expirado) para {area}")
