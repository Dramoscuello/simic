...
EMAIL_REGEX = r"^[^\s@]+@[^\s@]+\.[^\s@]+$"
NIT_REGEX = r"^\d{9,10}-\d{1}$"
PHONE_REGEX = r"^\d{7,10}$"
...
class AdminSetup(BaseModel):
    ...
    def validar_password(cls, v):
        if not re.fullmatch(PASSWORD_REGEX, v):
            raise ValueError("La contrasena debe tener al menos 8 caracteres, incluir letras, numeros y un caracter especial")
        return v
...
class InstitucionSetup(BaseModel):
    ...
    nit: str = Field(..., min_length=10, max_length=20)
    ...
    def validar_dane(cls, v):
        if not re.fullmatch(r"\d{12}", v):
            raise ValueError("El codigo DANE debe tener exactamente 12 digitos numericos")
        return v

    @field_validator("nit")
    def validar_nit(cls, v):
        if not re.fullmatch(NIT_REGEX, v):
            raise ValueError("El NIT debe tener el formato 900123456-7 (9 o 10 digitos, guion, digito verificador)")
        return v

    @field_validator("email_contacto")
    def validar_email_contacto(cls, v):
        if not re.fullmatch(EMAIL_REGEX, v):
            raise ValueError("El correo electronico de contacto no tiene un formato valido")
        return v

    @field_validator("email_rector")
    def validar_email_rector(cls, v):
        if not re.fullmatch(EMAIL_REGEX, v):
            raise ValueError("El correo electronico del rector no tiene un formato valido")
        return v

    @field_validator("telefono")
    def validar_telefono(cls, v):
        if v is not None and v.strip():
            if not re.fullmatch(PHONE_REGEX, v.strip()):
                raise ValueError("El telefono debe contener entre 7 y 10 digitos numericos")
        return v

    @field_validator("telefono_rector")
    def validar_telefono_rector(cls, v):
        if v is not None and v.strip():
            if not re.fullmatch(PHONE_REGEX, v.strip()):
                raise ValueError("El telefono del rector debe contener entre 7 y 10 digitos numericos")
        return v
