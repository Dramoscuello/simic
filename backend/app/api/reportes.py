...
def _get_grupal_batch_items(db: Session, inst_id: Optional[int] = None) -> List[ReporteItem]:
    ...
    for bid, sims in batch_map.items():
        ...
        rg_q = db.query(ReporteGrupal).filter(
            ReporteGrupal.simulacro_id.in_(sim_ids),
            ReporteGrupal.anulado.is_(False)
        )
        if inst_id:
            rg_q = rg_q.filter(ReporteGrupal.institucion_id == inst_id)
        reportes_grupales = rg_q.all()
        ...
        resp_map = {}
        for r in respuestas:
            if r.puntaje_total is not None:
                resp_map.setdefault(r.simulacro_id, []).append(float(r.puntaje_total))
        ...
        for s in sims:
            ...
            rg = rg_map.get(s.id)
            if rg:
                stats = rg.estadisticas_agregadas or {}
                raw_avg = stats.get("average_score_100")
                if raw_avg is not None:
                    avg_score = float(raw_avg)
                g_date = rg.created_at
                if latest_date is None or (g_date and g_date > latest_date):
                    latest_date = g_date

            if avg_score is None and s.id in resp_map:
                scores = resp_map[s.id]
                if scores:
