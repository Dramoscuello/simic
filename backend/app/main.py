    ...
    try:
        ...
        for role_name in roles:
            ...
            if not role:
                ...
                db.add(role)
                db.commit()
        # Sync/backfill usuarios.sede_id desde grupos.sede_id para consistencia
        from sqlalchemy import text
        db.execute(text("UPDATE usuarios SET sede_id = (SELECT sede_id FROM grupos WHERE grupos.id = usuarios.grupo_id) WHERE usuarios.grupo_id IS NOT NULL AND usuarios.sede_id IS NULL;"))
        db.commit()
    except Exception as e:
        print(f"Error inicializando roles/datos: {e}")
    finally:
        db.close()
