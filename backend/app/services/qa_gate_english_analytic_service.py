...
class GateEnglishAnalyticValidator:
    ...
    PART_MAX_LEVEL = {
        ...
        "Parte 6": "b1", # Inferencial (B1)
        "Parte 7": "b1"  # Gramática II (B1)
    }
    ...
    def _load_lexicon(cls):
        ...
        if not os.path.exists(csv_path):
            logger.warning(f" Gate Inglés: No se encontró {csv_path}. Saltando validación analítica.")
            return
...
"""
            ...
            if data.get("verdict") == "APPROVE":
                print(f"          Juez LLM: APELACIÓN ACEPTADA ({data.get('reason')})")
                return True
            else:
                print(f"          Juez LLM: CONFIRMA RECHAZO ({data.get('reason')})")
                return False
        ...
        for p in preguntas_ingles:
            ...
            if total_scorable > 0:
                ...
                if percent_off > 0.20:
                    ...
                    print(f"       ALERTA [Q{p.get('id')}] ({parte_key}): {msg_base}. Consultando Juez LLM...")
