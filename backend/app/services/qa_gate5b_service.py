        ...
        try:
            ...
            for p in preguntas:
                if not isinstance(p, dict):
                     print(f"       Ignorando elemento inválido en preguntas: {type(p)}")
                     continue
                ...
                if similitud < cls.THRESHOLD_ERROR:
                    ...
                    print(f"       Pregunta {pregunta_id}: similitud={similitud:.3f} (WARNING - relación débil)")
                else:
            ...
            if errores == 0:
                ...
                if warnings > 0:
                    print(f"       {warnings} pregunta(s) con relación débil (revisar)")
            else:
