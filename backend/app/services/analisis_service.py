...
"""
    ...
    @staticmethod
    def generar_reporte_global(batch_id: str, institucion_id: int, areas: list):
        """
        Genera reporte global multi-área para un batch de simulacros.
        Agrupa por estudiante y calcula puntaje por área + puntaje global ICFES.

        Fórmula global:
          ((3×Lectura) + (3×Mat) + (3×Soc) + (3×Nat) + (1×Inglés)) / 13 × 5
        """
        ...
        try:
            ...
            if not simulacros:
                return {"error": "No hay simulacros en este batch"}

            simulacro_ids = [s.id for s in simulacros]
            area_map = {s.id: s.area for s in simulacros}

            respuestas = db.query(RespuestaEstudiante).filter(
                RespuestaEstudiante.simulacro_id.in_(simulacro_ids),
                RespuestaEstudiante.institucion_id == institucion_id,
                RespuestaEstudiante.fecha_finalizacion.isnot(None),
                RespuestaEstudiante.anulado.is_(False)
            ).all()
            ...
            if not respuestas:
                return {"error": "No hay respuestas finalizadas en este batch"}

            user_ids = list(set(r.usuario_id for r in respuestas))
            usuarios = {u.id: u for u in db.query(Usuario).filter(Usuario.id.in_(user_ids)).all()}

            estudiantes_map = {}
            for r in respuestas:
                uid = r.usuario_id
                area = area_map.get(r.simulacro_id, "DESCONOCIDO")
                if uid not in estudiantes_map:
                    user = usuarios.get(uid)
                    grupo_nombre = user.grupo.nombre if user and user.grupo else "-"
                    estudiantes_map[uid] = {
                        "nombre": user.nombre if user else str(uid),
                        "documento": user.numero_documento if user else "-",
                        "grupo": grupo_nombre,
                        "areas": {},
                        "completas": 0,
                    }
                score = 0.0 if r.fraude else float(r.puntaje_total or 0)
                estudiantes_map[uid]["areas"][area] = score
                estudiantes_map[uid]["completas"] += 1
            ...
            for uid, data in estudiantes_map.items():
                row = {
                    "nombre": data["nombre"],
                ...
                for area in areas:
                    score = data["areas"].get(area)
                    row[area.lower()] = round(score, 2) if score is not None else None
                    if score is None:
                        tiene_todas = False
                    else:
                        peso = PESOS.get(area, 3)
                        suma_ponderada += score * peso

                if tiene_todas and len(areas) == len(PESOS):
                    row["puntaje_global"] = round((suma_ponderada / PESO_TOTAL) * 5, 2)
                    globales.append(row["puntaje_global"])
                else:
                    row["puntaje_global"] = None

                estudiantes_reporte.append(row)

            estudiantes_reporte.sort(key=lambda e: e["nombre"])

            promedios_por_area = {}
            for area in areas:
                scores = [e[area.lower()] for e in estudiantes_reporte if e[area.lower()] is not None]
                promedios_por_area[area_display.get(area, area)] = round(sum(scores) / len(scores), 2) if scores else None

            promedio_global = round(sum(globales) / len(globales), 2) if globales else None

            areas_incluidas = sorted(set(
                area_map.get(s.id, "DESCONOCIDO") for s in simulacros
            ))
            areas_faltantes = sorted(
                a for a in ["MATEMATICAS", "LECTURA_CRITICA", "CIENCIAS_NATURALES", "SOCIALES_CIUDADANAS", "INGLES"]
                if a not in areas_incluidas
            )

            institucion_nombre = ""
            inst = simulacros[0].institucion if simulacros[0].institucion else None
            if inst:
                institucion_nombre = inst.nombre

            metadata = {
                "tipo_reporte": "global_batch",
                "batch_id": batch_id,
                "institucion_id": institucion_id,
                "institution_name": institucion_nombre,
                "areas_incluidas": areas_incluidas,
                "areas_faltantes": areas_faltantes,
                "areas_display": area_display,
                "total_estudiantes": len(estudiantes_reporte),
                "promedios_por_area": promedios_por_area,
                "promedio_global": promedio_global,
                "formula_global": "((3×Lectura) + (3×Mat) + (3×Soc) + (3×Nat) + (1×Inglés)) / 13 × 5",
            ...
            logger.error(f"Error reporte global: {e}")
            return {"error": str(e)}
        finally:
            db.close()
