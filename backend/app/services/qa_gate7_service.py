...
"""
    ...
    def validar_matematicas(
        ...
        if not wolfram_app_id:
            print(f"\n Gate 7:  WOLFRAM_APP_ID no configurado - Gate deshabilitado")
            result.stats["mensaje"] = "Gate 7 deshabilitado: institución no tiene Wolfram App ID"
            return result

        if not openai_api_key:
            print(f"\n Gate 7:  OPENAI_API_KEY no configurado - Gate deshabilitado")
        ...
        try:
            ...
            if errores == 0:
                ...
                print(f"    Gate 7: {errores} errores, {validadas_ok} correctas ({elapsed:.2f}s)")
