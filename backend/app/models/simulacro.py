...
class Simulacro(Base):
    ...
    # Agrupación por batch (vincula áreas de un mismo "simulacro lógico")
    ...
    reviews = relationship("ReviewPregunta", back_populates="simulacro", cascade="all, delete-orphan", passive_deletes=True)
    respuestas_estudiantes = relationship("RespuestaEstudiante", back_populates="simulacro", cascade="all, delete-orphan", passive_deletes=True)
    reportes_grupales = relationship("ReporteGrupal", back_populates="simulacro", cascade="all, delete-orphan", passive_deletes=True)
    preguntas_registradas = relationship("PreguntaUsada", back_populates="simulacro", cascade="all, delete-orphan", passive_deletes=True)
