...
def _patch_notification_side_effects(monkeypatch):
    monkeypatch.setattr("app.api.mensajeria.create_notifications_bulk", lambda *args, **kwargs: None)
    monkeypatch.setattr("app.api.mensajeria.get_admins", lambda *args, **kwargs: [])
    monkeypatch.setattr("app.api.mensajeria.get_admins_institucion", lambda *args, **kwargs: [])
