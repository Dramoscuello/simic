...
echo "[entrypoint] Iniciando Reaper en background (cada 5 min)..."
(
  while true; do
    sleep 300
    cd /app && PYTHONPATH=/app python -m app.tasks.close_expired_attempts >> /var/log/SIMIC/reaper.log 2>&1
  done
) &
echo "[entrypoint] Reaper iniciado (PID $!)."
