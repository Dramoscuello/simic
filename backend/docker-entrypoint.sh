...
*/5 * * * * root cd /app && HOST_POSTGRES=${HOST_POSTGRES} PORT_POSTGRES=${PORT_POSTGRES} USER_POSTGRES=${USER_POSTGRES} PASS_POSTGRES=${PASS_POSTGRES} DB_POSTGRES=${DB_POSTGRES} REDIS_URL=${REDIS_URL} PYTHONPATH=/app /usr/local/bin/python -m app.tasks.close_expired_attempts >> /var/log/SIMIC/reaper.log 2>&1
