...
def parse_markdown_patterns(content, area):
    ...
    if not matches:
        logger.warning(f" No se encontraron patrones con formato estándar en {area}")
        return []
...
def main():
    ...
    try:
        ...
        logger.info(f" Colección '{COLLECTION_NAME}' eliminada (Clean Slate).")
    except ValueError:
        logger.info(f"ℹ La colección '{COLLECTION_NAME}' no existía.")
        ...
        for rel_path in file_paths:
            ...
            for i in range(0, len(ids), BATCH_SIZE):
                ...
                try:
                    ...
                    logger.error(f"    Error insertando lote {i}-{end}: {e}")
