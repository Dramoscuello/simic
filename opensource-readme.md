...
### 0. Instalar PostgreSQL 16 + pgvector y Redis
...
sudo systemctl enable redis-server && sudo systemctl start redis
```
...
WSL2 te permite ejecutar un entorno Linux completo dentro de Windows, lo cual es la vía más estable para PostgreSQL, pgvector y Redis.

1. Instala WSL2:
   ```powershell
   wsl --install
   ```
   Reinicia el equipo si es la primera instalación. Esto instala Ubuntu por defecto.

2. Abre la terminal de Ubuntu (WSL) y sigue las instrucciones de [Linux](#-linux-ubuntudebian) arriba.

3. Si PostgreSQL no arranca con systemctl, inícialo manualmente:
   ```bash
   sudo service postgresql start
   sudo service redis-server start
   ```

> Con WSL2, todo el resto de la guía (backend, frontend, scripts) se ejecuta desde la terminal de Ubuntu. No necesitas adaptar ningún comando.

---

**Opción B — PostgreSQL nativo en Windows + pgvector**

1. Descarga el instalador de [PostgreSQL 16 para Windows](https://www.enterprisedb.com/downloads/postgresql-postgresql-downloads) (versión 16.x para Windows x86-64). Durante la instalación:
   - Anota la contraseña del superusuario `postgres`.
   - Puerto por defecto: `5432`.
   - Opcional: marca Stack Builder al finalizar (no es necesario para pgvector).

2. Agrega `bin` de PostgreSQL al PATH. Abre PowerShell como administrador y ejecuta:
   ```powershell
   [Environment]::SetEnvironmentVariable("Path", $env:Path + ";C:\Program Files\PostgreSQL\16\bin", "Machine")
   ```
   Cierra y reabre la terminal para que el cambio surta efecto.

3. Instala pgvector manualmente:
   - Descarga la DLL precompilada desde [pgvector releases](https://github.com/pgvector/pgvector/releases) (busca el `.zip` para Windows x64 correspondiente a tu versión de PostgreSQL, ej. `pgvector-0.7.4-postgresql-16-x64-windows.zip`).
   - Extrae el ZIP. Copia los archivos a la carpeta de instalación de PostgreSQL:
     ```
     *.dll          → C:\Program Files\PostgreSQL\16\lib\
     *.control, *.sql  → C:\Program Files\PostgreSQL\16\share\extension\
     ```
   - Si la carpeta `extension` no existe, créala.
...
4. Reinicia el servicio de PostgreSQL:
   ...
   ```
   > El nombre exacto del servicio puede variar. Verifícalo con `services.msc` o con `Get-Service *postgres*` en PowerShell.

5. Crea la base de datos y habilita la extensión vector. Abre PowerShell o CMD:
   ```powershell
   psql -U postgres
   ```
   Luego ejecuta:
...
**Opción C — Redis para Windows**

Redis no tiene soporte oficial para Windows. Estas son las alternativas:

- **Redis for Windows** (fork mantenido por tporadowski):
  ```powershell
  # Descarga el .msi desde https://github.com/tporadowski/redis/releases
  # Instala con las opciones por defecto (puerto 6379)
  # El instalador MSI lo configura como servicio de Windows automáticamente
  ```
...
- **Memurai** (alternativa comercial con edición gratuita para desarrollo):
  Descarga desde [memurai.com](https://www.memurai.com/get-memurai).

---

> **Nota para Windows nativo (sin WSL2):** en los pasos siguientes, reemplaza `python3` por `python` y usa `venv\Scripts\activate` en lugar de `source .venv/bin/activate`. Para comandos de shell que usan `&&`, ejecútalos línea por línea en PowerShell/CMD.
...
En producción se recomienda un cron cada 5 minutos (VPS) o el loop background automático si usas Docker. En desarrollo puedes ejecutarlo manualmente:
...
8. Configurar el Reaper (cron en VPS, loop background en Docker).
