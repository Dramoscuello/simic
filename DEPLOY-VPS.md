...
- Branch objetivo: `main` (repo publico).
- Las claves de OpenAI/Claude y demás proveedores IA se configuran exclusivamente en `.env`. El asistente `/setup` solo registra la institución y el admin.
...
- `CLAUDE_API_KEY` — clave de API de Anthropic Claude
- `OPENAI_BASE_URL` — endpoint compatible con API OpenAI (opcional, ej. Ollama local)
- `DEFAULT_GENERATION_MODEL` — modelo por defecto para generación (default `o3`)
- `WOLFRAM_APP_ID` — clave para validación matemática
...
# Proveedores de IA — se configuran exclusivamente aquí (variables de entorno).
# Si falta alguna clave, las funciones que la requieran fallarán gracefulmente.
OPENAI_API_KEY=sk-...
...
2. Crear automáticamente la "Sede Principal" asociada a la institución.
3. Crear el usuario administrador.

Las API keys de IA no se configuran aquí, sino en `backend/.env`.
