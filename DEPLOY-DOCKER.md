...
- Backend FastAPI (contenedor propio, `python:3.12-slim` + uvicorn + loop background para Reaper)
...
- Reaper automatico via loop background dentro del contenedor backend
...
- Servidor con **Docker Engine 24+** y **Docker Compose v2** (plugin `docker compose`, no el standalone `docker-compose`).
- Git.
- Repositorio publico: `https://github.com/Dramoscuello/simic.git` (rama `main`).
  Si usas el repositorio privado, la rama es `libre`.
- Puerto 80 (o `FRONTEND_PORT`) abierto en el firewall del servidor.
...
| `OPENAI_API_KEY` | API key OpenAI | (requiere cambio) |
| `CLAUDE_API_KEY` | API key Anthropic | (requiere cambio) |
...
|---|---|---|
| `OPENAI_BASE_URL` | Endpoint API compatible (Ollama local, proxy) | — |
...
cd simic
git checkout main        # rama principal del repo publico
```

> Si usas el repositorio privado (`simicfes.git`), la rama de trabajo es `libre`,
> no `main`. La rama `main` del repo publico se regenera desde `libre` en cada release.
...
> `FRONTEND_URL` debe coincidir con la URL desde donde se accede a la app.
> Si aun no tienes dominio, usa la IP publica del servidor: `http://190.xxx.xxx.xxx`.
> Si pruebas en local: `http://localhost`. Cuando consigas dominio, cambialo
> y recrea el backend (`docker compose up -d backend`).

Las variables de IA en `.env` **son obligatorias** para que funcione la generacion
de simulacros y el bootstrap RAG. El asistente `/setup` NO configura API keys;
solo registra la institucion y el admin.
...
3. Configurar al menos un modelo de IA (nombre visible y codigo exacto, ej: `o3`, `gpt-4o-mini`).

Las API keys de OpenAI, Claude y Wolfram **deben estar en el `.env`**,
no se configuran desde el asistente web.
...
```bash
# Backend directo (interno, usando Python)
docker compose exec backend python -c "import urllib.request; print(urllib.request.urlopen('http://localhost:8000/').read().decode())"
...
El Reaper corre automaticamente via un loop background dentro del contenedor.
Se ejecuta cada 5 minutos.

Verificar que el Reaper esta activo:

```bash
docker compose logs backend | grep -i reaper
```
...
```bash
mkdir -p ./backups
BACKUP_FILE="simic_backup_$(date +%Y%m%d_%H%M%S).dump"

docker compose exec postgres pg_dump \
  -U "${USER_POSTGRES:-simic}" \
  -d "${DB_POSTGRES:-simic_db}" \
  --format=custom \
  --file="/tmp/${BACKUP_FILE}"

docker compose cp \
  "postgres:/tmp/${BACKUP_FILE}" \
  "./backups/${BACKUP_FILE}"
```
...
- [ ] Login funcional
- [ ] Reaper loop activo (`docker compose logs backend | grep -i reaper`)
