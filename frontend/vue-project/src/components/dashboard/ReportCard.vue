  ...
  >
     ...
     <div class="pl-1">
       ...
       <div class="flex items-center gap-2 mt-auto">
               <span v-if="score !== undefined && score !== null" :class="getScoreClassPct(score)" class="text-xs font-bold px-1.5 py-0.5 rounded">
                   {{ score.toFixed(1) }}
               </span>
           ...
           <!-- Area badges (batch reports) -->
           <div v-if="areas && areas.length" class="flex flex-wrap gap-1 mt-2 pl-1">
               <span
                   v-for="area in areas"
                   :key="area.area"
                   :class="area.fraude ? 'bg-rose-100 dark:bg-rose-900/40 text-rose-600 dark:text-rose-400' : getScoreClass(area.score)"
                   class="text-[10px] font-medium px-1.5 py-0.5 rounded flex items-center gap-1"
               >
                   <span v-if="area.fraude" class="material-icons-round text-[12px]">gavel</span>
                   {{ area.display }}
                   <span v-if="area.score !== null && area.score !== undefined" class="font-bold">{{ area.score.toFixed(0) }}</span>
               </span>
           </div>
     </div>
  </div>
...
const props = defineProps({
    ...
    score: Number,
    tags: Array,
    areas: Array,  // [{ area, display, score, fraude }] — para reportes agrupados
    badge: String,
    icon: String,
    fraude: Boolean,
    color: {
...
const getScoreClassPct = (raw) => {
    const pct = raw > 100 ? (raw / 500) * 100 : raw;
    return getScoreClass(pct);
};
</script>
