  ...
  <div class="mx-auto w-full max-w-7xl flex flex-col gap-6 relative p-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
      ...
      <div v-if="isAdmin" class="flex items-center gap-2">
        ...
        <button
          @click="openBulkModal()"
          class="flex items-center justify-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg px-4 py-2.5 text-sm font-medium transition-colors shadow-sm"
        >
    ...
    <!-- Modal Carga Masiva -->
    <div v-if="bulkModal.show" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
      <div class="bg-white dark:bg-slate-800 w-full max-w-lg rounded-2xl shadow-xl overflow-hidden animate-in fade-in zoom-in duration-200">
        <div class="px-6 py-4 border-b border-slate-100 dark:border-slate-700 flex justify-between items-center bg-slate-50 dark:bg-slate-700/50">
            <h3 class="text-lg font-bold text-slate-800 dark:text-slate-100">Carga Masiva de Estudiantes</h3>
            <button @click="closeBulkModal" class="text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 transition-colors">
                <span class="material-icons-round">close</span>
            </button>
        </div>
        ...
        <div class="p-6 space-y-4">
            <div>
                <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Grupo <span class="text-rose-500">*</span></label>
                <Select
                  v-model="bulkModal.grupoId"
                  :options="grupos"
                  optionLabel="nombre"
                  optionValue="id"
                ...
                >
                  ...
                  <template #value="slotProps">
                      <div v-if="slotProps.value" class="text-slate-900 dark:text-white">{{ getGrupoLabel(slotProps.value) }}</div>
                      <span v-else class="text-slate-500 dark:text-slate-400">{{ slotProps.placeholder }}</span>
                  </template>
                </Select>
            </div>

            <div class="bg-indigo-50 dark:bg-indigo-900/30 p-3 rounded-lg border border-indigo-100 dark:border-indigo-800 text-xs">
                ...
                <div class="bg-white dark:bg-slate-800 p-2 rounded border border-indigo-100 dark:border-indigo-900/50">
                    ...
                    <code class="text-indigo-600 dark:text-indigo-400 font-mono block">password <span class="text-slate-400 dark:text-slate-500 text-[10px]">(Mín. 8 caracteres. Letras + números + carácter especial)</span></code>
                    <code class="text-indigo-600 dark:text-indigo-400 font-mono block">tipo_documento <span class="text-slate-400 dark:text-slate-500 text-[10px]">(CC, RC, TI)</span></code>
                    <code class="text-indigo-600 dark:text-indigo-400 font-mono block">numero_documento</code>
                </div>
            </div>

            <div>
                <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Archivo Excel</label>
                <input 
                    type="file" 
                    ref="bulkFileInput" 
                    @change="onBulkFileChange" 
                    accept=".xlsx" 
                    class="block w-full text-sm text-slate-500 dark:text-slate-400 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-primary/10 file:text-primary hover:file:bg-primary/20"
                />
                <p v-if="bulkModal.fileName" class="mt-1 text-xs text-slate-500 dark:text-slate-400">
                    Archivo seleccionado: <span class="font-medium text-slate-700 dark:text-slate-300">{{ bulkModal.fileName }}</span>
                </p>
            </div>
        </div>

        <div class="px-6 py-4 border-t border-slate-100 dark:border-slate-700 flex justify-end gap-3 bg-slate-50 dark:bg-slate-700/50">
            <button type="button" @click="closeBulkModal" class="px-4 py-2 text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-lg font-medium transition-colors">
                Cancelar
            </button>
            <button 
                @click="handleBulkUpload" 
                :disabled="bulkModal.uploading || !bulkModal.grupoId || !bulkModal.file" 
                class="px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg font-medium shadow-sm transition-all disabled:opacity-50 flex items-center gap-2"
            >
                <span v-if="bulkModal.uploading" class="animate-spin h-4 w-4 border-2 border-white rounded-full border-t-transparent"></span>
                {{ bulkModal.uploading ? 'Procesando...' : 'Procesar' }}
            </button>
        </div>
      </div>
    </div>
...
const openBulkModal = () => {
    bulkModal.value.show = true;
    bulkModal.value.grupoId = null;
    bulkModal.value.file = null;
    bulkModal.value.fileName = '';
    bulkModal.value.uploading = false;
};

const closeBulkModal = () => {
    bulkModal.value.show = false;
    bulkModal.value.grupoId = null;
    bulkModal.value.file = null;
    bulkModal.value.fileName = '';
    bulkModal.value.uploading = false;
...
const onBulkFileChange = (event) => {
    ...
    bulkModal.value.file = file;
    bulkModal.value.fileName = file ? file.name : '';
};

const handleBulkUpload = async () => {
    if (!bulkModal.value.file || !bulkModal.value.grupoId) return;

    const selectedGrupo = grupos.value.find(g => g.id === bulkModal.value.grupoId);
    if (!selectedGrupo) {
        toast.add({ severity: 'error', summary: 'Error', detail: 'Grupo no encontrado', life: 4000 });
        return;
    }

    const institucionId = selectedGrupo.institucion_id;
    if (!institucionId) {
        toast.add({ severity: 'error', summary: 'Error', detail: 'No se pudo determinar la institución del grupo', life: 4000 });
        return;
    }

    bulkModal.value.uploading = true;
    try {
        const formData = new FormData();
        formData.append('file', bulkModal.value.file);
        formData.append('rol_nombre', 'estudiante');
        formData.append('grupo_id', bulkModal.value.grupoId);

        const result = await usuariosStore.importUsers(institucionId, formData);

        const creados = result.creados || 0;
        const errores = result.errores || [];
        const total = result.total_procesados || 0;

        if (errores.length > 0) {
            toast.add({ 
                severity: 'warn', 
                summary: 'Importación completada con errores', 
                detail: `${creados} creados de ${total}. ${errores.length} errores.`,
                life: 6000 
            });
        } else {
            toast.add({ severity: 'success', summary: 'Importación completada', detail: `${creados} estudiantes creados`, life: 4000 });
        }

        closeBulkModal();
        await fetchEstudiantes(buildFilters());
    } catch (e) {
        const detail = e.response?.data?.detail || e.message || 'Error en importación';
...
watch(userInstitutionId, async (newInstId) => {
    if (newInstId) {
        await Promise.all([
            gruposStore.fetchGrupos(newInstId, selectedSede.value || undefined),
            sedesStore.fetchSedes(newInstId),
            fetchEstudiantes(buildFilters())
        ]);
    }
});
...
onMounted(async () => {
   ...
   try {
       await fetchRoles();
       if (isInstitucionAdmin.value && userInstitutionId.value) {
           await Promise.all([
               gruposStore.fetchGrupos(userInstitutionId.value, selectedSede.value || undefined),
               sedesStore.fetchSedes(userInstitutionId.value),
               fetchEstudiantes(buildFilters())
           ]);
       } else if (isAdmin.value) {
           await Promise.all([
               gruposStore.fetchGrupos(null, selectedSede.value || undefined),
