  ...
  <div class="mx-auto flex w-full max-w-7xl flex-col gap-6">
    ...
    <div v-if="userRole === 'admin' || userRole === 'admin'" class="bg-white dark:bg-slate-800 rounded-xl p-4 shadow-sm border border-slate-100 dark:border-slate-700">
      ...
      <div class="flex flex-wrap items-center gap-3">
        ...
        <!-- Filtro por Lote (Batch ID) -->
        <div v-if="userRole === 'admin'" class="flex flex-col gap-1">
          <label class="text-xs text-slate-500 dark:text-slate-400">Lote (batch)</label>
          <Select 
            v-model="filterBatch" 
...
const hasFilters = computed(() => {
    return filterSede.value || filterStatus.value || filterArea.value || filterBatch.value;
});
...
// Watcher de Batch para Admin
watch(filterBatch, async (newVal) => {
    if (newVal) {
...
watch(userRole, async (val) => {
    if (val && val !== 'guest') {
        await Promise.all([
            loadOptions(),
            fetchSimulacros(),
            val === 'admin' ? fetchBatches() : Promise.resolve()
        ]);
    }
}, { immediate: true });
...
watch([searchQuery, filterArea, filterStatus, filterSede, filterBatch], () => {
    currentPage.value = 1;
});
...
const filteredSimulacros = computed(() => {
  ...
  if (filterBatch.value) {
    result = result.filter(s => s.batch_id === filterBatch.value);
  }
...
const countByStatus = computed(() => ({
  activo: filteredSimulacros.value.filter(s => s.estado === 'activo').length,
  finalizado: filteredSimulacros.value.filter(s => s.estado === 'finalizado').length
}));
...
const statsCardCount = computed(() => {
  if (userRole.value === 'estudiante' && activeTab.value === 'realizado') return filteredSimulacros.value.length;
