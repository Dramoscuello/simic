<template>
  <div class="mx-auto flex w-full max-w-7xl flex-col gap-6">
    ...
    <!-- Batch Global Report Card -->
    <!-- Diagnóstico Global (Multi-área) - Siempre visible para admin -->
    <div 
        v-if="userRole === 'admin' && batches.length > 0" 
        class="border rounded-xl p-4 flex items-center justify-between mb-4 bg-gradient-to-r from-violet-50 to-purple-50 dark:from-violet-900/20 dark:to-purple-900/20 border-violet-200 dark:border-violet-800 animate-in fade-in"
    >
        <div class="flex items-center gap-3">
            <div class="p-2 rounded-lg bg-violet-100 dark:bg-violet-800 text-violet-700 dark:text-violet-300">
                <span class="material-icons-round">auto_awesome</span>
            </div>
            <div>
                <h4 class="font-bold text-violet-900 dark:text-violet-100">Diagnóstico Global (Multi-área)</h4>
                <p class="text-sm text-violet-700 dark:text-violet-300">
                    {{ batches.length }} batch{{ batches.length !== 1 ? 'es' : '' }} disponible{{ batches.length !== 1 ? 's' : '' }}
                    <span v-if="incompleteBatchesCount > 0" class="text-amber-600 dark:text-amber-400 font-medium">
                        — {{ incompleteBatchesCount }} incompleto{{ incompleteBatchesCount !== 1 ? 's' : '' }}
                    </span>
        ...
        <div class="flex gap-2 items-center">
            <router-link
                v-if="incompleteBatchesCount > 0"
                to="/simulacros/crear"
                class="text-xs text-amber-700 dark:text-amber-300 bg-amber-100 dark:bg-amber-900/30 px-3 py-1.5 rounded-lg hover:bg-amber-200 dark:hover:bg-amber-900/50 transition-colors font-medium flex items-center gap-1"
            >
                <span class="material-icons-round text-sm">add_circle</span>
                Completar lotes pendientes
            </router-link>
            <Select
                v-model="selectedGlobalBatchId"
            ...
            />
            <button 
                @click="generateGlobalBatchReport"
                :disabled="!selectedGlobalBatchId || globalBatchReportLoading"
                class="bg-violet-600 hover:bg-violet-700 shadow-violet-500/20 text-white px-5 py-2.5 rounded-lg text-sm font-bold shadow-md transition-all flex items-center gap-2 disabled:opacity-50"
            >
                <span v-if="globalBatchReportLoading" class="animate-spin rounded-full h-4 w-4 border-2 border-white/50 border-t-white"></span>
                <span v-else class="material-icons-round">assessment</span>
                Generar diagnóstico global
            </button>
        </div>
    </div>
    ...
    <!-- Global Batch Report Modal -->
    <Dialog 
        v-model:visible="showGlobalBatchModal"
        :modal="true"
        :header="'Diagnóstico Global Multi-área'"
        :style="{ width: '95vw', maxWidth: '1200px' }"
        :breakpoints="{ '960px': '75vw', '640px': '100vw' }"
        :pt="{ root: { class: 'mx-2' }, content: { class: 'p-0' } }"
    >
        <div v-if="globalBatchReport" class="p-6 space-y-6 max-h-[75vh] overflow-y-auto">
            <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
                <div class="rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-4">
                    <p class="text-xs uppercase tracking-wide text-slate-500">Institución</p>
                    <p class="text-sm font-bold text-slate-900 dark:text-white mt-1">{{ globalBatchReport.institution_name || 'N/A' }}</p>
                </div>
                <div class="rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-4">
                    <p class="text-xs uppercase tracking-wide text-slate-500">Estudiantes</p>
                    <p class="text-sm font-bold text-slate-900 dark:text-white mt-1">{{ globalBatchReport.total_estudiantes || 0 }}</p>
                </div>
                <div class="rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-4">
                    <p class="text-xs uppercase tracking-wide text-slate-500">Promedio Global</p>
                    <p class="text-sm font-bold text-violet-600 dark:text-violet-400 mt-1">{{ formatScore(globalBatchReport.promedio_global) }} / 500</p>
                </div>
                <div class="rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-4">
                    <p class="text-xs uppercase tracking-wide text-slate-500">Generado</p>
                    <p class="text-xs font-bold text-slate-900 dark:text-white mt-1">{{ globalBatchReport.generated_at || 'N/A' }}</p>
                </div>
            </div>
                    ...
                    <table class="min-w-full text-sm border-collapse">
                        <thead>
                            <tr class="bg-violet-600 text-white">
                                <th class="px-2 py-2 text-center font-bold w-10">#</th>
                                <th class="px-2 py-2 text-left font-bold">Estudiante</th>
                                <th class="px-2 py-2 text-left font-bold w-24">Grupo</th>
                                <th v-for="area in globalBatchReport.areas_incluidas" :key="area" class="px-2 py-2 text-center font-bold w-20">
                                    {{ (globalBatchReport.areas_display || {})[area] || area.substring(0, 4) }}
                                </th>
                                <th class="px-2 py-2 text-center font-bold w-24 bg-violet-700">Puntaje Global</th>
                            </tr>
                            <tr class="bg-violet-50 dark:bg-violet-900/20">
                                <th colspan="2" class="px-2 py-1 text-right text-xs text-slate-500">Promedios</th>
                                <th class="px-2 py-1 text-center text-xs text-slate-500">—</th>
                                <th v-for="area in globalBatchReport.areas_incluidas" :key="'avg_'+area" class="px-2 py-1 text-center text-xs font-bold text-slate-600 dark:text-slate-300">
                                    {{ formatScore((globalBatchReport.promedios_por_area || {})[(globalBatchReport.areas_display || {})[area] || area]) }}
                                </th>
                                <th class="px-2 py-1 text-center text-xs font-bold text-violet-700 dark:text-violet-300 bg-violet-100 dark:bg-violet-900/30">
                                    {{ formatScore(globalBatchReport.promedio_global) }}
                                </th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr
                                v-for="(student, idx) in (globalBatchReport.estudiantes || [])"
                                :key="idx"
                                class="odd:bg-slate-50 dark:odd:bg-slate-800 even:bg-white dark:even:bg-slate-900 border-b border-slate-100 dark:border-slate-700"
                            >
                                <td class="px-2 py-2 text-center font-semibold text-slate-600 dark:text-slate-300 text-xs">{{ idx + 1 }}</td>
                                <td class="px-2 py-2 text-slate-900 dark:text-slate-100 font-medium text-xs">{{ student.nombre }}</td>
                                <td class="px-2 py-2 text-xs text-slate-600 dark:text-slate-300">{{ student.grupo }}</td>
                                <td v-for="area in globalBatchReport.areas_incluidas" :key="area" class="px-2 py-2 text-center text-xs">
                                    <span v-if="student[area.toLowerCase()] !== null && student[area.toLowerCase()] !== undefined"
                                        class="inline-flex items-center rounded-full bg-indigo-100 dark:bg-indigo-900/40 text-indigo-700 dark:text-indigo-300 px-2 py-0.5 text-xs font-bold">
                                        {{ formatScore(student[area.toLowerCase()]) }}
                                    </span>
                                    <span v-else class="text-slate-300">—</span>
                                </td>
                                <td class="px-2 py-2 text-center text-xs">
                                    <span v-if="student.puntaje_global !== null && student.puntaje_global !== undefined"
                                        class="inline-flex items-center rounded-full bg-violet-100 dark:bg-violet-900/40 text-violet-700 dark:text-violet-300 px-2 py-1 text-xs font-bold">
                                        {{ formatScore(student.puntaje_global) }}
                                    </span>
                                    <span v-else class="text-slate-300">—</span>
                                </td>
                            </tr>
                            <tr v-if="!(globalBatchReport.estudiantes || []).length">
                                <td :colspan="3 + (globalBatchReport.areas_incluidas?.length || 0)" class="px-3 py-5 text-center text-slate-500">Sin estudiantes para mostrar.</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
            ...
            <div v-if="globalBatchReport.formula_global" class="text-xs text-slate-500 dark:text-slate-400 text-center">
                Fórmula puntaje global: {{ globalBatchReport.formula_global }}
            </div>
        </div>
        <div v-else class="p-6 text-center text-slate-400">Cargando...</div>
    </Dialog>
...
// Global Batch Report Logic
const batches = ref([]);
const selectedGlobalBatchId = ref(null);
const globalBatchReportLoading = ref(false);
const showGlobalBatchModal = ref(false);
const globalBatchReport = ref(null);

const selectPtBatch = {
  root: {
    class: 'w-full rounded-lg border border-violet-200 dark:border-violet-600 bg-white dark:bg-slate-800 text-slate-800 dark:text-white'
  },
  label: {
  ...
  panel: {
    class: 'bg-white dark:bg-slate-700 border border-violet-200 dark:border-violet-600'
  },
  item: {
    class: 'text-slate-700 dark:text-white hover:bg-violet-50 dark:hover:bg-violet-900/30'
  }
};

const batchSelectOptions = computed(() => {
  return batches.value.map(batch => ({
    label: `${batch.titulo_base} (${batch.areas_cubiertas?.length || 0}/${5} áreas — ${batch.completo ? 'Completo' : 'Incompleto'})`,
...
const fetchBatches = async () => {
  if (userRole.value !== 'admin') return;
  try {
    const response = await api.get('/simulacros/batches');
...
const incompleteBatchesCount = computed(() => {
  return batches.value.filter(b => !b.completo).length;
});

const generateGlobalBatchReport = async () => {
  if (!selectedGlobalBatchId.value) return;
  globalBatchReportLoading.value = true;
  try {
    const response = await api.post(`/simulacros/batches/${selectedGlobalBatchId.value}/reporte-global`);
    globalBatchReport.value = response.data;
    showGlobalBatchModal.value = true;
  } catch (e) {
    const detailMsg = e?.response?.data?.detail || 'No se pudo generar el reporte global';
    toast.add({ severity: 'error', summary: 'Error', detail: detailMsg, life: 4000 });
  } finally {
    globalBatchReportLoading.value = false;
  }
};
