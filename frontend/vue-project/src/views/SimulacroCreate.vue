      ...
      <div class="p-6 space-y-8" :class="{ 'opacity-50 pointer-events-none': isGenerating }">
        ...
        <div>
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <!-- Lotes incompletos (visible siempre en modo creación) -->
            <div v-if="!isEditMode && incompleteBatches.length > 0 && !selectedBatchId" class="md:col-span-2">
              <div class="flex items-center gap-2 mb-3">
                <label class="text-sm font-medium text-slate-700 dark:text-slate-300">
                  Lotes incompletos ({{ incompleteBatches.length }})
                </label>
                <span class="text-xs text-amber-600 dark:text-amber-400 bg-amber-50 dark:bg-amber-900/30 px-2 py-0.5 rounded-full font-medium">
                  Tienen áreas pendientes por completar
                </span>
              </div>
              <div class="space-y-2 max-h-48 overflow-y-auto">
                <button
                  v-for="batch in incompleteBatches"
                  :key="batch.batch_id"
                  type="button"
                  @click="selectBatch({ ...batch, num_preguntas: batch.num_preguntas || 30 })"
                  class="w-full text-left px-4 py-3 rounded-lg border-2 transition-all duration-200"
                  :class="selectedBatchId === batch.batch_id
                    ? 'border-primary bg-primary/5'
                    : 'border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700/40 hover:border-primary/50'"
                >
                  <div class="flex items-center justify-between">
                    <div>
                      <p class="font-medium text-slate-800 dark:text-slate-200 text-sm">{{ batch.titulo_base }}</p>
                      <p class="text-xs text-slate-500 dark:text-slate-400 mt-1">
                        {{ new Date(batch.fecha_creacion).toLocaleDateString() }}
                      </p>
                    </div>
                    <span class="text-xs font-medium text-amber-600 dark:text-amber-400 bg-amber-50 dark:bg-amber-900/30 px-2 py-1 rounded-full">
                      {{ batch.areas_cubiertas?.length || 0 }}/5 áreas
                    </span>
                  </div>
                  <div class="flex flex-wrap gap-1 mt-2">
                    <span
                      v-for="area in batch.areas_cubiertas"
                      :key="area"
                      class="text-xs px-2 py-0.5 rounded-full bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-300"
                    >{{ getAreaLabel(area) }}</span>
                    <span
                      v-for="area in batch.areas_faltantes"
                      :key="area"
                      class="text-xs px-2 py-0.5 rounded-full bg-slate-100 dark:bg-slate-700 text-slate-400 dark:text-slate-500"
                    >{{ getAreaLabel(area) }}</span>
                  </div>
                </button>
              </div>
            ...
            <!-- Batch Continuation (SOLO EN CREACIÓN) -->
            <div v-if="!isEditMode && batchSuggestions.length > 0" class="md:col-span-2">
              <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">
                Continuar simulacro existente
              </label>
              <div class="space-y-2">
                <button
                  v-for="batch in batchSuggestions"
                  :key="batch.batch_id"
                  type="button"
                  @click="selectBatch(batch)"
                  class="w-full text-left px-4 py-3 rounded-lg border-2 transition-all duration-200"
                  :class="{
                    'border-primary bg-primary/5': selectedBatchId === batch.batch_id,
                    'border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700/40 hover:border-primary/50': selectedBatchId !== batch.batch_id
                  }"
                >
                  <div class="flex items-center justify-between">
                    <div>
                      <p class="font-medium text-slate-800 dark:text-slate-200 text-sm">{{ batch.titulo_base }}</p>
                      <p class="text-xs text-slate-500 dark:text-slate-400 mt-1">
                        Creado: {{ new Date(batch.fecha_creacion).toLocaleDateString() }}
                      </p>
                    </div>
                    <span class="text-xs font-medium text-amber-600 dark:text-amber-400 bg-amber-50 dark:bg-amber-900/30 px-2 py-1 rounded-full">
                      Faltan {{ batch.areas_faltantes?.length || 0 }} áreas
                    </span>
                  </div>
                  <div class="flex flex-wrap gap-1 mt-2">
                    <span
            ...
            <!-- Selected batch info -->
            <div v-if="!isEditMode && selectedBatchId" class="md:col-span-2">
              <div class="flex items-center gap-2 p-3 bg-indigo-50 dark:bg-indigo-900/20 rounded-lg border border-indigo-200 dark:border-indigo-800/50">
                <span class="material-icons-round text-indigo-600 dark:text-indigo-400 text-lg">link</span>
                <p class="text-sm text-indigo-700 dark:text-indigo-300">
                  Continuando batch — las áreas ya cubiertas no se regenerarán.
                </p>
                <button
                  type="button"
                  @click="clearBatchSelection()"
                  class="ml-auto text-xs text-indigo-500 hover:text-indigo-700 dark:hover:text-indigo-300 underline"
                >Desvincular</button>
              </div>
            </div>
            ...
            <div v-if="!isEditMode">
              ...
              <div v-if="selectedBatchId">
                <p class="text-xs text-amber-600 dark:text-amber-400 mt-1 flex items-center gap-1">
                  ...
                  Fijado por el batch existente (no modificable)
                </p>
              </div>
              <p v-else-if="sedeOptions.length > 1" class="text-xs text-slate-400 dark:text-slate-500 mt-1">Selecciona una o varias sedes que presentarán el simulacro</p>
            ...
            <div v-if="!isEditMode">
              ...
              <Select
                ...
                :disabled="!!selectedBatchId"
              />
              <p v-if="selectedBatchId" class="text-xs text-amber-600 dark:text-amber-400 mt-1 flex items-center gap-1">
                <span class="material-icons-round text-[14px]">lock</span>
                Fijado por el batch existente ({{ form.num_preguntas }} preguntas)
              </p>
              <p v-else class="text-xs text-slate-400 dark:text-slate-500 mt-1">Opciones permitidas: 10, 20 o 30 preguntas por simulacro</p>
            </div>
        ...
        <div v-if="!isEditMode">
          ...
          <div class="flex flex-wrap gap-3">
            <button
              ...
              :class="{
                'bg-slate-200 dark:bg-slate-700/60 text-slate-400 dark:text-slate-500 border-slate-300 dark:border-slate-600 cursor-not-allowed': selectedBatchId && selectedBatchAreasCubiertas.includes(area.value),
                'border-primary bg-primary text-white shadow-md shadow-primary/25': isAreaSelected(area.value) && !(selectedBatchId && selectedBatchAreasCubiertas.includes(area.value)),
                'border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700/40 text-slate-600 dark:text-slate-300 hover:border-slate-300 dark:hover:border-slate-500 hover:bg-slate-50 dark:hover:bg-slate-700': !isAreaSelected(area.value) && !(selectedBatchId && selectedBatchAreasCubiertas.includes(area.value))
              }"
              :disabled="selectedBatchId && selectedBatchAreasCubiertas.includes(area.value)"
            >
              <span class="material-icons-round text-[18px]">
                <template v-if="selectedBatchId && selectedBatchAreasCubiertas.includes(area.value)">
                  lock
                </template>
                <template v-else>
          ...
          <div class="bg-slate-50 dark:bg-slate-700/30 rounded-xl p-4 border border-slate-200 dark:border-slate-600 space-y-3">
            ...
            <div class="flex justify-between items-start">
              ...
              <div class="text-right">
                <span class="block text-sm font-bold text-primary">{{ areasNuevas.length }} Áreas</span>
                <span class="text-xs text-slate-400 dark:text-slate-500 block max-w-[200px] truncate">
                  {{ areasNuevas.map(a => getAreaLabel(a)).join(', ') }}
                </span>
              </div>
            </div>
...
const selectedBatchAreasCubiertas = ref([]);
const batchDebounceTimer = ref(null);

const incompleteBatches = ref([]);
const loadingIncompleteBatches = ref(false);

const fetchIncompleteBatches = async () => {
  if (isEditMode.value) return;
  const instId = form.institucion_id || (isAdminIE.value && authStore.user?.institucion_id);
  if (!instId) {
    incompleteBatches.value = [];
    return;
  }
  loadingIncompleteBatches.value = true;
  try {
    const response = await api.get('/simulacros/batches', {
      params: { institucion_id: instId }
    });
    const allBatches = response.data || [];
    incompleteBatches.value = allBatches.filter(b => !b.completo);
  } catch {
    incompleteBatches.value = [];
  } finally {
    loadingIncompleteBatches.value = false;
  }
};

const fetchBatchSuggestions = async (nombreBase) => {
  if (!nombreBase || nombreBase.trim().length < 3 || isEditMode.value) {
    batchSuggestions.value = [];
    return;
  }
  try {
    const response = await api.get('/simulacros/batch-suggestions', {
      params: { nombre_base: nombreBase.trim() }
    });
    batchSuggestions.value = response.data || [];
  } catch {
    batchSuggestions.value = [];
  }
};
...
const selectBatch = (batch) => {
  ...
  selectedBatchAreasCubiertas.value = batch.areas_cubiertas || [];
  form.titulo = batch.titulo_base || form.titulo;
  form.num_preguntas = batch.num_preguntas || form.num_preguntas;
  form.sede_ids = batch.sede_ids || form.sede_ids;
  form.tiempo_limite = batch.duracion_minutos || form.tiempo_limite;
  // Auto-seleccionar áreas faltantes
  const faltantes = batch.areas_faltantes || [];
  form.areas = [...(batch.areas_cubiertas || [])];
  faltantes.forEach(area => {
...
const clearBatchSelection = () => {
  ...
  selectedBatchAreasCubiertas.value = [];
};
...
const areasNuevas = computed(() => {
  return form.areas.filter(
    area => !(selectedBatchId.value && selectedBatchAreasCubiertas.value.includes(area))
  );
});

const totalSimulacrosGenerados = computed(() => {
  const sedesCount = form.sede_ids?.length || (sedeOptions.value.length === 1 ? 1 : 0);
  return areasNuevas.value.length * sedesCount;
});
...
const toggleArea = (areaValue) => {
  if (selectedBatchId.value && selectedBatchAreasCubiertas.value.includes(areaValue)) {
    return;
  }
...
const generarSimulacros = async () => {
  ...
  const areasParaGenerar = form.areas.filter(
    area => !(selectedBatchId.value && selectedBatchAreasCubiertas.value.includes(area))
  );

  if (areasParaGenerar.length === 0) {
    toast.add({ severity: 'warn', summary: 'Sin áreas nuevas', detail: 'Todas las áreas seleccionadas ya existen en este batch.', life: 4000 });
    return;
  }
  ...
  // Inicializar progreso - solo para las áreas que se van a generar
  areasParaGenerar.forEach(area => {
...
watch(() => form.titulo, (newVal) => {
  if (batchDebounceTimer.value) clearTimeout(batchDebounceTimer.value);
  if (selectedBatchId.value) return;
  batchDebounceTimer.value = setTimeout(() => {
    fetchBatchSuggestions(newVal);
  }, 400);
});
